#!/bin/bash
# Train ACT (Action Chunking with Transformers) - CVAE + Transformer encoder-decoder.
#
# Usage:
#   bash scripts/train_act.sh DATA_PATH SAVE_PATH GPU
#
# Examples:
#   # Multi-task
#   bash scripts/train_act.sh data/easy_mode checkpoints/act 3
#
#   # Single-task
#   bash scripts/train_act.sh data/easy_mode/0 checkpoints/act_0 3
#
# Environment variables (override defaults):
#   BATCH_SIZE=128  EPOCHS=100  LR=1e-4  NUM_WORKERS=8
#   EXP_NAME=act  TRAIN_SUFFIX=_train  TEST_SUFFIX=_test
#   REPR_TYPE=img-pos  (change to img-depth-pos to add depth)
#   INSTR_MODE=""  INSTRUCTIONS_FILE=workflow/instructions.json
#   CACHE_ON_GPU=0
#   LAZY_LOADING=1  (load images on-demand; requires uncompressed NPZ)
#   USE_WANDB=1 WANDB_PROJECT=TexasPoker_ACT WANDB_ENTITY=<entity>
...
WANDB_PROJECT=${WANDB_PROJECT:-"TexasPoker_ACT"}
...
echo ""
echo "============================================================"
echo "  ACT  (ResNet18 + CVAE Transformer)"
echo "============================================================"
...
CMD=(python train.py
    ...
    # ACT-specific (paper defaults: hidden=512, heads=8, ff=3200)
    --act_hidden_dim 512
    --act_num_heads 8
    --act_latent_dim 32
    --act_kl_weight 10.0
